...
One more thing that you should pay attention to is to avoid using so many abbreviations in your method or variable names. Proper naming is not easy sometimes, but it will be very beneficial to other people reading your code, and your future se




Im really glad you mentioned proper naming. Often, I will shorten the method names by using abbreviations to stay under the Ruby style guide 80 character limit. I would definitely prefer to name a method like the example below. 

How can I comply with style guide and give methods proper names?

``ruby
def detect_players_fourth_square_winning_strategy_sequence(plyr_sequence_recorder)
  disrupt_player_strategy = BLOCK_THIS_EXACT_SEQUENCE.select do |blk_seq|
...
def computer_places_piece!(brd, xoro, plyr_sequence_recorder)
  ...
  if computer_offense_defense!(brd, xoro).class == Integer
    brd[computer_offense_defense!(brd, xoro)] = xoro
    return
  elsif detect_players_fourth_square_winning_strategy_sequence(plyr_sequence_recorder).class == Integer
    brd[detect_players_fourth_square_winning_strategy_sequence(plyr_sequence_recorder)] = xoro
    return
  elsif computer_ai_logic!(brd, xoro).class == Integer
    brd[computer_ai_logic!(brd, xoro)] = xoro
  end
